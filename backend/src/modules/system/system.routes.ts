  ...
  exec('git --version', (gitError) => {
    if (gitError) {
      return res.status(400).json({
        code: 'GIT_NOT_FOUND',
        message: 'Git is not installed on this host. Update the project from the host terminal, then rebuild the frontend and backend before restarting the 9Drive process.'
      })
    }
